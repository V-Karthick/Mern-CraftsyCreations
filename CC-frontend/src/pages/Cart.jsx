...
const Cart = () => {
  ...
  <div className="h-full w-full bg-black flex flex-col justify-center items-center overflow-y-auto">
  <h1 className="text-3xl font-extralight text-left h-[15%] text-yellow-400">YOUR CART</h1>

  <div className="h-[90%] w-[90%] flex justify-center items-center">
    <div className="h-full w-full flex flex-wrap items-center justify-center gap-8">
      {cartItems.map(({ _id, userEmail, productImage, productName, productDesc, productPrice }) => (
        <div
          key={_id}
          className="shadow-md w-[80%] rounded-lg p-6 flex flex-row justify-center items-center 
                     group transform transition-all hover:-translate-y-2 bg-[#1a1a1a] gap-8 
                     hover:bg-[#2a2a2a]"
        >
          {/* Product Image */}
          <div className="w-[30%]">
            <img
              src={productImage}
              alt={productName}
              className="w-80 h-48 object-cover mb-4 rounded"
            />
          </div>
          ...
          <div className="w-2/3 pl-6 flex flex-col justify-between">
            <div className="w-full">
              <h2 className="text-xl font-semibold group-hover:text-yellow-400 text-yellow-300">
                {productName}
              </h2>

              <p className="text-gray-300 group-hover:text-white mb-0">{productDesc}</p>
              <span className="text-4xl font-bold text-yellow-400">{productPrice}</span>
            </div>

            {/* Buttons */}
            <div className="flex justify-end mt-1 items-end gap-3">
              <button
                onClick={() => orderClickHandler(productName, productImage, productPrice)}
                className="bg-yellow-400 hover:bg-yellow-500 text-black w-[20%] rounded-md px-2 py-2 font-bold"
              >
                Order Now!
              </button>
              <button
                onClick={() => removeHandler(userEmail, _id)}
                className="text-black font-bold py-2 px-4 rounded bg-red-500 hover:bg-red-600"
              >
                REMOVE FROM CART
              </button>
            </div>
  ...
  {/* Order Page Modal */}
  {orderPage && (
    <div className="h-screen w-screen absolute top-0 left-0 bg-black/40 flex justify-center items-center z-50">
      <div className="h-[70%] w-[40%] border-2 border-yellow-400 bg-[#111] flex flex-col justify-center items-center rounded-xl">
        
        {/* Modal Header */}
        <div className="h-[13%] w-full bg-yellow-400 text-black border-b-2 border-yellow-500 rounded-t-xl flex items-center justify-center">
          <div className="h-full w-[80%] flex justify-between items-center">
            <h2 className="text-black text-2xl font-semibold">Place Your Order</h2>
            <X
              onClick={() => setOrderPage(false)}
        ...
        <form
          ...
          className="flex flex-col justify-center items-center h-[90%] w-[90%] gap-4"
        >
          <input
            className="w-[80%] px-4 py-2 border-b-2 bg-transparent text-yellow-300 
                       focus:outline-none border-yellow-400 hover:border-yellow-500"
            type="text"
          ...
          <button
            className="bg-yellow-400 text-black w-[30%] rounded-md px-4 py-2 font-bold hover:bg-yellow-500"
            type="submit"
          >
            Confirm Order
          </button>
